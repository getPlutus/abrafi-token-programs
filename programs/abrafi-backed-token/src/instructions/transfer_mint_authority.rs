...
pub fn transfer_mint_authority_handler(ctx: Context<TransferMintAuthority>) -> Result<()> {
    ...
    // Validate that minting is disabled (required before transferring mint authority)
    require!(
        !state.is_minting_enabled,
        ErrorCode::InvalidConfiguration
    );

    // Validate that request unminting is disabled (required before transferring mint authority)
    require!(
        !state.is_unminting_request_enabled,
        ErrorCode::InvalidConfiguration
    );
