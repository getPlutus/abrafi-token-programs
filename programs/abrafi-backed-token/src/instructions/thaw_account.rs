...
pub struct ThawAccount<'info> {
    ...
    /// User's abrafi token account to thaw
    ...
    #[account(
        ...
        constraint = user_abrafi_backed_token_account.is_frozen() @ ErrorCode::AccountNotFrozen,
    )]
...
pub fn thaw_account_handler(ctx: Context<ThawAccount>) -> Result<()> {
    ...
    token::thaw_account(CpiContext::new_with_signer(
        ...
        token::ThawAccount {
            account: ctx.accounts.user_abrafi_backed_token_account.to_account_info(),
    ...
    emit!(AccountThawedEvent {
        version: 1,
        token_account: ctx.accounts.user_abrafi_backed_token_account.key(),
        user: ctx.accounts.user_abrafi_backed_token_account.owner,
    });
